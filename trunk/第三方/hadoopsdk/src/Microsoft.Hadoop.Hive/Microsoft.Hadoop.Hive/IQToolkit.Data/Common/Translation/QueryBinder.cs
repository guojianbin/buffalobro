...
namespace IQToolkit.Data.Common
{
    ...
    public class QueryBinder : DbExpressionVisitor
    {
        ...
        protected override Expression VisitMethodCall(MethodCallExpression m)
        {
            if (m.Method.DeclaringType == typeof(HiveExtensionMethods))
            {
                switch (m.Method.Name)
                {
                    ...
                    case "FlatMap":
                        return this.BindFlatMap(m.Type, m.Arguments[0], GetLambda(m.Arguments[1]));
                    case "ClusterBy":
        ...
        private Expression BindMap(Type resultType, Expression source, LambdaExpression lambda)
        {
            ...
            var newProjector = this.mapper.GetEntityExpression(newSelect, entity);
            ...
            //var outputType = TypeHelper.GetElementType(resultType);
            //MappingEntity entity = this.mapper.Mapping.GetEntity(outputType);
            //var newProjection = this.VisitSequence(this.mapper.GetQueryExpression(entity));
            //this.map[Expression.Parameter(outputType)] = newProjection.Projector;
            ...
            //ProjectedColumns pc = this.ProjectColumns(projection.Projector, newProjection.Select.Alias, projection.Select.Alias);
            //var map = new MapExpression(newProjection.Select.Alias, newProjection.Select.Columns, "MAP", driver_path, exe_path, classname, methodname);

            //return new ProjectionExpression(
            //    new SelectExpression(newProjection.Select.Alias, pc.Columns, projection.Select, null, null, null, null, false, map, null, null, false),
            //    newProjection.Projector
            //    );
            ...
            //ProjectedColumns pc = this.ProjectColumns(projection.Projector, /*newProjection.Select.Columns,*/ /*newProjection.Select.Alias*/alias, projection.Select.Alias);
            //var map = new MapExpression(/*newProjection.Select.Alias*/alias, newProjection.Select.Columns, driver_path, exe_path, classname, methodname);

            //return new ProjectionExpression(
            //    new SelectExpression(newProjection.Select.Alias, /*projection.Select.Columns*/pc.Columns, projection.Select, null, null, null, false, map, null, null, false),
            //    /*pc.Projector*/newProjection.Projector
            //    );        
        }

        private Expression BindFlatMap(Type resultType, Expression source, LambdaExpression lambda)
        {
            ProjectionExpression projection = this.VisitSequence(source);
            this.map[lambda.Parameters[0]] = projection.Projector;
            ...
            this.map[Expression.Parameter(outputType)] = newProjector;
            ProjectedColumns newpc = this.ProjectColumns(newProjector, alias, alias, projection.Select.Alias);
            var map = new MapExpression(alias, newpc.Columns, "FLATMAP", driver_path, exe_path, classname, methodname);
            ...
            //ProjectedColumns pc = this.ProjectColumns(projection.Projector, newProjection.Select.Alias, projection.Select.Alias);
            //var map = new MapExpression(newProjection.Select.Alias, newProjection.Select.Columns, "FLATMAP", driver_path, exe_path, classname, methodname);
