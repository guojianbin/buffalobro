@CheckIn
Feature: Set-AzureHDInsightDefaultStorage Cmdlet
		 In order to manage my HDInsight clusters on my Azure subscription
		 As an IT professional
		  I want to be able to set the default storage account for a complex configurations.

Background: I have setup the Cmdlets
  	  Given I have installed the AzureHDInsight Cmdlets
        And I am using the "Set-AzureHDInsightDefaultStorage" PowerShell Cmdlet

Scenario: There Exists a Get-AzureHDInsightCluster PowerShell Cmdlet
	 Then There exists a "Set-AzureHDInsightDefaultStorage" PowerShell Cmdlet

Scenario: There is only 1 ParameterSet specified for the Get-AzureHDInsightCluster PowerShell Cmdlet
	 Then there exists a "Set Default Storage Account" parameter set
...
Scenario: I can use the "Set Default Storage Account" parameter set
	 When I am using the "Set Default Storage Account" parameter set
	 Then there exists a "Config" Cmdlet parameter
	  ...
	  And   it can not take its value from the pipeline 
	  And there exists a "StorageContainerName" Cmdlet parameter
	  And   it is of type "String"
	  And   it is a required parameter
