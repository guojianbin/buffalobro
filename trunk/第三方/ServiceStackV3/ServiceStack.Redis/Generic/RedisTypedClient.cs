...
namespace ServiceStack.Redis.Generic
{
		...
		: IRedisTypedClient<T>
	{
		...
		public IList<T> GetByIds(IEnumerable ids)
		{
			if (ids != null)
			{
                var urnKeys = ids.ConvertAllTo(x => client.UrnKey<T>(x));
				if (urnKeys.Count != 0)
		...
		public void DeleteByIds(IEnumerable ids)
		{
			...
			var urnKeys = ids.ConvertAllTo(t => client.UrnKey<T>(t));
            if (urnKeys.Count > 0)
            {
                this.RemoveEntry(urnKeys.ToArray());
                client.RemoveTypeIds<T>(ids.ConvertAllTo(x => x.ToString()).ToArray());
            }
		}
