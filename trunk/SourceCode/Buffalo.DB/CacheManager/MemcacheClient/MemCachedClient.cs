...
namespace Memcached.ClientLibrary
{
	...
	public class MemcachedClient 
	{
		...
		{
			return Delete(key, null,TimeSpan.MaxValue );
		}
		...
		/// <returns><c>true</c>, if the data was deleted successfully</returns>
		public bool Delete(string key, TimeSpan expiry) 
		{
		...
		/// <param name="expiry">when to expire the record.</param>
		/// <returns><c>true</c>, if the data was deleted successfully</returns>
		public bool Delete(string key, object hashCode, TimeSpan expiry) 
		{
			if(key == null) 
			{
		...
		/// <returns></returns>
		private static int GetExpirationTime(TimeSpan expiration)
		{
            ...
            return (int)expiration.TotalSeconds;
		}
		...
		{
            return Set("set", key, value, TimeSpan.MaxValue, null, _primitiveAsString);
		}
		...
		{
            return Set("set", key, value, TimeSpan.MaxValue, hashCode, _primitiveAsString);
		}
		...
		/// <param name="hashCode">if not null, then the int hashcode to use</param>
		/// <returns>true, if the data was successfully stored</returns>
        public bool Set(string key, object value, TimeSpan expiry, int hashCode) 
		{
		...
		{
            return Set("add", key, value, TimeSpan.MaxValue, null, _primitiveAsString);
		}
		...
		{
            return Set("add", key, value, TimeSpan.MaxValue, hashCode, _primitiveAsString);
		}
		...
		/// <param name="hashCode">if not null, then the int hashcode to use</param>
		/// <returns>true, if the data was successfully stored</returns>
        public bool Add(string key, object value, TimeSpan expiry, int hashCode) 
		{
		...
		{
            return Set("replace", key, value, TimeSpan.MaxValue, null, _primitiveAsString);
		}
		...
		{
            return Set("replace", key, value, TimeSpan.MaxValue, hashCode, _primitiveAsString);
		}
		...
		/// <param name="hashCode">if not null, then the int hashcode to use</param>
		/// <returns>true, if the data was successfully stored</returns>
        public bool Replace(string key, object value, TimeSpan expiry, int hashCode) 
		{
        ...
        private bool Set(string cmdname, string key, object obj, TimeSpan expiry, object hashCode, bool asString) 
		{
		...
		{
            return Set("set", key, counter, TimeSpan.MaxValue, null, true);
		}
		...
		{
            return Set("set", key, counter, TimeSpan.MaxValue, hashCode, true);
		}
				...
				if(line.StartsWith(VALUE)) 
				{
					...
					if((flag & F_SERIALIZED) == 0) 
					{
						...
						else 
						{
							...
							catch(Exception e) 
							{
								...
								throw new IOException(("loaditems deserialize error").Replace("$$Key$$", key), e);
							}
						}
					}
					else 
					{
						...
						catch(SerializationException e) 
						{
							...
							throw new IOException(("loaditems SerializationException").Replace("$$Key$$", key), e);
						}
					}
